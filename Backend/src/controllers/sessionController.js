...
const getActiveSessions = asyncHandler( async (_, res) => {
...
const getMyRecentSessions = asyncHandler( async (req, res) => {
    ...
    res.status(200).json(
        new ApiResponse(
            200,
            { sessions },
            "Users Recent Sessions fetched successfully"
        )
    )
});
...
const joinSession = asyncHandler( async (req, res) => {
    ...
    if(!session){
        throw new ApiError(404, "Session is not found");
    }
    ...
    if(session.host.toString() === userId.toString()){
        throw new ApiError(400, "Host cannot join their own session as participant");
    }
